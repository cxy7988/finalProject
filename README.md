...
## 1. Introduction（简介）
- 项目背景与目标简述
- 系统名称：**Course & Professor Rating System**
...
- 数据查询（Query 部分） 
  - 按课程代码 / 课程名查询：显示课程信息、授课教授、平均评分、所有评论
  - 按教授姓名查询：显示他教的课程、每门课的平均分、总体平均分、评论
  - 查看教授评分排名（Top N）：按平均分排序并输出 —— 这里用排序算法
...
- `HashMap<String, Course> courseMap`
    ...
    - 用途：作为**主索引结构**，支持按课程编号快速查找课程
    - 这是系统中**最频繁的查询方式**（用户通常以课程为入口），因此用哈希表优化课程查询性能

- `ArrayList<Course> courseList`
    - 存储系统中所有课程的顺序列表
    - 用途：
        - 遍历所有课程（例如：按课程名关键字做线性查找）
        - 按课程编号排序后，支持二分查找算法
    - 作为排序、统计、全局扫描等操作的基础存储结构

- `List<CourseProfessor>`（每个 `Course` 内部）
    - 对于每门课程，使用 `List<CourseProfessor>` 保存该课程下的所有教授及其评分信息
    - 当用户按课程查询时，从这个列表中取出所有教授并进行排序，得到“该课程下教授评分排名”

- `List<Rating>`（每个 `CourseProfessor` 内部）
    - 保存某门课上某位教授的全部评分记录
    - 用于计算该课程-教授组合的平均分
...
- `List<CourseProfessor>`（课程内部教授列表）
    - 优点：
        ...
        - 与排序算法结合简单，可以直接对 `List<CourseProfessor>` 进行选择排序
    - 在本系统中，用于：
...
### 4.3 Selection Sort（选择排序，用于课程内教授评分排名）

- 用途：在**某一门课程内部**，对这门课下的所有教授按评分从高到低排序
- 数据结构：`List<CourseProfessor>`（来自 `course.getProfessorList()`）
- 实现思路：
    - 对 `professorList` 做选择排序：
        1. 外层循环从 `i = 0` 到 `n - 2`
        2. 找出 `[i, n-1]` 区间中 `getAverageRating()` 最大的下标 `maxIndex`
        3. 交换 `professorList.get(i)` 和 `professorList.get(maxIndex)`
- 时间复杂度：O(n²)
- 在系统中的应用场景：
    - 用户输入课程 ID → 找到该课程 → 获取 `professorList` → 使用选择排序 → 输出**“该课程下教授评分排名”**
---
